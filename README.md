...
- **70 tools across six services** — see the [catalog](#tools) below.
...
| `drive_comment_create` | Start a new comment thread, optionally quoting text. On a Google Doc the quote must exist (the reply says which section it's in); the Drive API can't highlight it in the Doc. |
| `drive_comment_resolve` | Resolve a thread (or reopen it), optionally with a closing reply. |
| `drive_revision_list` / `drive_revision_get` | A file's revision history; for a Google Doc, `get` returns that revision's content (text by default). |
...
| Tool | What it does |
|---|---|
| `docs_create` | Create a Google Doc, optionally in a folder and already filled from Markdown — or copy a **template** and fill its `{{placeholders}}`. |
| `docs_get` | Text — or **Markdown** with its formatting, for rewrites that keep it — plus tabs, tables, `revision_id` and a heading **outline** with each section's index range: how Claude finds where to edit. Includes headers, footers, footnotes and smart chips; `suggestions_view_mode` shows the doc with suggestions inline, accepted or rejected. |
| `docs_insert` | Insert **Markdown** (headings, bold/italic, code, links, bullets, numbered lists, checklists, **tables**, code blocks, `>` indented paragraphs, `<!-- pagebreak -->` and `<!-- sectionbreak -->`) as native Docs formatting — at the start or end, after a heading's section, or at an index. |
...
| `docs_table_edit` | Add or delete a table's rows and columns, set a cell's text, merge/unmerge cells, shade cells, pin header rows, set a column width or row height. |
| `docs_image` | List a tab's images, replace one (URL or local file) or delete it — inline or positioned. |
| `docs_insert_chip` | Insert a smart chip — a person, a date or a Google Drive link — placed like a footnote. |
| `docs_header_footer` | Set or delete the default header or footer. |
| `docs_footnote` | Add a footnote after a quoted phrase, a heading's section, or an index. |
| `docs_page_setup` | Page size (letter/A4/legal), orientation, margins and page colour; per section: margins and columns. |
| `docs_tab` | Add, rename or delete a document tab. |
| `docs_named_range` | Create, list, delete, or replace the text of a named range. |
...
│   ├── server.py                    # the MCP server — all 70 tools
